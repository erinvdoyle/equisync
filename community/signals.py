...
def notify_admin_new_ad(sender, instance, created, **kwargs):
    if created and not instance.approved:
        message = f" New ad submitted by {instance.user.username} — needs approval"
...
def notify_admin_new_announcement(sender, instance, created, **kwargs):
    if created and not instance.approved:
        message = f" New announcement from {instance.user.username} — pending approval"
        for approver in get_approvers():
            send_notification(approver, message)
...
def notify_admin_new_event(sender, instance, created, **kwargs):
    if created and not instance.approved:
        message = f" New event: '{instance.title}' — awaiting approval"
        for approver in get_approvers():
            send_notification(approver, message)
...
def notify_user_ad_approved(sender, instance, created, **kwargs):
    if not created and instance.approved and not getattr(instance, '_was_approved', False):
        send_notification(instance.user, f"Your ad '{instance.title or 'Ad'}' has been approved!")

@receiver(post_save, sender=Announcement)
def notify_user_announcement_approved(sender, instance, created, **kwargs):
    if not created and instance.approved and not getattr(instance, '_was_approved', False):
        send_notification(instance.user, "Your announcement has been approved!")

@receiver(post_save, sender=CommunityEvent)
def notify_user_event_approved(sender, instance, created, **kwargs):
    if not created and instance.approved and not getattr(instance, '_was_approved', False):
        send_notification(instance.created_by, f"Your community event '{instance.title}' has been approved!")

@receiver(post_save, sender=CompetitionEvent)
def notify_user_competition_event_approved(sender, instance, created, **kwargs):
    if not created and instance.approved and not getattr(instance, '_was_approved', False):
        send_notification(instance.created_by, f"Your competition event '{instance.title}' has been approved!")
