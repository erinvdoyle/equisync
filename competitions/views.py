...
def calendar_view(request, year=None, month=None):
    if request.GET.get('from') == 'horse_profile':
        ...
        has_event_results = EventHorse.objects.filter(
...
def week_view(request, year, month, day):
    ...
    for i in range(7):
        ...
        if search_term:
            events_for_day = events_for_day.filter(
                Q(title__icontains=search_term) |
                Q(description__icontains=search_term)
            )
...
def edit_event(request):
    ...
    return render(
        request,
        'competitions/edit_event.html',
        {'event': event, 'upcoming_events': upcoming_events}
    )
    ...
    data = [
        {
            ...
            'url': reverse('competitions:event_detail', args=[event.id])
        } for event in events
    ]
...
def edit_event_horse(request, event_horse_id):
    ...
    if request.method == 'POST':
        event_horse.class_details = request.POST.get(
            'class_details', event_horse.class_details)
        event_horse.notes = request.POST.get(
            'notes', event_horse.notes)
        event_horse.results = request.POST.get(
            'results', event_horse.results)
        event_horse.performance_rating = request.POST.get(
            'performance_rating', event_horse.performance_rating)

        jump_height_value = request.POST.get('jump_height')
        if jump_height_value and jump_height_value != "Other":
            try:
                event_horse.jump_height = float(
                    jump_height_value.replace('m', '').replace(
                        'cm', '').strip()
                )
        ...
        event_horse.save()
        return redirect(
            'competitions:event_detail', event_id=event_horse.event.id)

    return render(
        request, 'competitions/edit_event_horse.html',
...
def remove_event_horse(request, event_horse_id):
    ...
    return render(
        request, 'competitions/remove_event_horse.html', {
            'event_horse': event_horse}
    )
...
def horse_results_archive(request, horse_id):
    ...
    return render(
        request, 'competitions/horse_results_archive.html',
        {'horse': horse, 'results': results}
    )
