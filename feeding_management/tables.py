...
class HorseLinkColumn(tables.Column):
    def render(self, value, record):
        url = reverse('feeding_management:horse_feeding_chart_readonly', kwargs={'horse_id': record.horse.id})
        return format_html('<a href="{}" class="horse-name-link">{}</a>', url, value)
...
class FeedingChartTable(tables.Table):
    ...
    hay = tables.Column(accessor='get_hay_info', verbose_name="Hay", orderable=False)
    supplements = tables.Column(accessor='supplements', verbose_name="Supplements", orderable=False)
    medicines = tables.Column(accessor='medicines', verbose_name="Medicines", orderable=False)
    notes = tables.Column(accessor='notes', verbose_name="Notes", orderable=False)
