...
class ExerciseScheduleAdmin(admin.ModelAdmin):
    list_display = ('horse', 'date', 'get_exercise_types', 'get_durations', 'created_by')
    ...
    def get_exercise_types(self, obj):
        return ", ".join([item.exercise_type for item in obj.schedule_items.all()])
    get_exercise_types.short_description = 'Exercise Types'

    def get_durations(self, obj):
        return ", ".join([str(item.duration) for item in obj.schedule_items.all()])
    get_durations.short_description = 'Durations'

    def get_readonly_fields(self, request, obj=None):
        if not (request.user.is_staff or self.has_owner_rider_permission(request.user, obj)):
    ...
    def has_add_permission(self, request):
        return request.user.is_staff or self.has_owner_rider_permission(request.user)

    def has_change_permission(self, request, obj=None):
        return request.user.is_staff or self.has_owner_rider_permission(request.user, obj)
    ...
    def has_owner_rider_permission(self, user, obj=None):
        ...
        if obj:
            ...
            return ExerciseSchedule.objects.filter(Q(horse__owner=user) | Q(horse__riders=user)).exists()
